...
Definition hSet : Type := total2 (λ X : Type, isaset X).
Definition make_hSet (X : Type) (i : isaset X) := tpair isaset X i : hSet.
Definition pr1hSet : hSet -> Type := @pr1 Type (λ X : Type, isaset X).
...
Definition forall_hSet {X : Type} (Y : X -> hSet) : hSet
...
Weak axiom of choice for [X] is the condition that for any family [P : X -> Type]
...
Definition ischoicebase_uu1 (X : Type)
  := ∏ P : X -> Type, (∏ x : X, ishinh (P x)) -> ishinh (∏ x : X, P x).

(** Uses RR1 *)
Lemma isapropischoicebase (X : Type) : isaprop (ischoicebase_uu1 X).
Proof.
  apply impred.
...
Definition ischoicebase (X : Type) : hProp := make_hProp _ (isapropischoicebase X).

Lemma ischoicebaseweqf {X Y : Type} (w : X ≃ Y) (is : ischoicebase X) :
  ischoicebase Y.
Proof.
...
Lemma ischoicebaseweqb {X Y : Type} (w : X ≃ Y) (is : ischoicebase Y) :
  ischoicebase X.
Proof.
...
Lemma ischoicebasecoprod {X Y : Type}
...
Definition hsubtype (X : Type) : Type := X -> hProp.
...
Definition make_carrier {X : Type} (A : hsubtype X) :
...
Definition totalsubtype (X : Type) : hsubtype X := λ x, htrue.
...
Definition weq_subtypes {X Y : Type} (w : X ≃ Y)
...
Definition subtypesdirprod {X Y : Type} (A : hsubtype X) (B : hsubtype Y) :
  hsubtype (X × Y) := λ xy : _, hconj (A (pr1 xy)) (B (pr2 xy)).

Definition fromdsubtypesdirprodcarrier {X Y : Type}
...
Definition tosubtypesdirprodcarrier {X Y : Type}
           (A : hsubtype X) (B : hsubtype Y)
...
Lemma weqsubtypesdirprod {X Y : Type} (A : hsubtype X) (B : hsubtype Y) :
...
Lemma ishinhsubtypedirprod  {X Y : Type} (A : hsubtype X) (B : hsubtype Y)
...
Definition squash_pairs_to_set {Y : Type} (F : Y -> Type) :
...
Definition squash_to_set {X Y : Type} (is : isaset Y) (f : X -> Y) :
          (∏ x x', f x = f x') -> ∥ X ∥ -> Y.
Proof.
  intros e w.
  set (P := ∑ y, ∃ x, f x = y).
...
Definition hrel (X : Type) : Type := X -> X -> hProp.
Identity Coercion idhrel : hrel >-> Funclass.

Definition brel (X : Type) : Type := X -> X -> bool.
...
Definition istrans {X : Type} (R : hrel X) : Type
  := ∏ (x1 x2 x3 : X), R x1 x2 -> R x2 x3 -> R x1 x3.

Definition isrefl {X : Type} (R : hrel X) : Type
  := ∏ x : X, R x x.

Definition issymm {X : Type} (R : hrel X) : Type
  := ∏ (x1 x2 : X), R x1 x2 -> R x2 x1.

Definition ispreorder {X : Type} (R : hrel X) : Type := istrans R × isrefl R.

Definition iseqrel {X : Type} (R : hrel X) := ispreorder R × issymm R.
Definition iseqrelconstr {X : Type} {R : hrel X}
           (trans0 : istrans R) (refl0 : isrefl R) (symm0 : issymm R) :
  iseqrel R := make_dirprod (make_dirprod trans0 refl0) symm0.

Definition isirrefl {X : Type} (R : hrel X) : Type := ∏ x : X, ¬ R x x.

Definition isasymm {X : Type} (R : hrel X) : Type
  := ∏ (x1 x2 : X), R x1 x2 -> R x2 x1 -> empty.

Definition iscoasymm {X : Type} (R : hrel X) : Type := ∏ x1 x2, ¬ R x1 x2 -> R x2 x1.

Definition istotal {X : Type} (R : hrel X) : Type := ∏ x1 x2, R x1 x2 ∨ R x2 x1.

Definition isdectotal {X : Type} (R : hrel X) : Type := ∏ x1 x2, R x1 x2 ⨿ R x2 x1.

Definition iscotrans {X : Type} (R : hrel X) : Type
  := ∏ x1 x2 x3, R x1 x3 -> R x1 x2 ∨ R x2 x3.

Definition isdeccotrans {X : Type} (R : hrel X) : Type
  := ∏ x1 x2 x3, R x1 x3 -> R x1 x2 ⨿ R x2 x3.

Definition isdecrel {X : Type} (R : hrel X) : Type := ∏ x1 x2, R x1 x2 ⨿ ¬ R x1 x2.

Definition isnegrel {X : Type} (R : hrel X) : Type
...
Definition isantisymm {X : Type} (R : hrel X) : Type
  := ∏ (x1 x2 : X), R x1 x2 -> R x2 x1 -> x1 = x2.

Definition isPartialOrder {X : Type} (R : hrel X) : Type
  := ispreorder R × isantisymm R.

Ltac unwrap_isPartialOrder i :=
  induction i as [transrefl antisymm]; induction transrefl as [trans refl].

Definition isantisymmneg {X : Type} (R : hrel X) : Type
  := ∏ (x1 x2 : X), ¬ R x1 x2 -> ¬ R x2 x1 -> x1 = x2.

Definition iscoantisymm {X : Type} (R : hrel X) : Type
...
Definition neqchoice {X : Type} (R : hrel X) : Type
...
Lemma istransandirrefltoasymm {X : Type} {R : hrel X}
...
Lemma isantisymmnegtoiscoantisymm {X : Type} {R : hrel X}
...
Lemma rtoneq {X : Type} {R : hrel X} (is : isirrefl R) {a b : X} (r : R a b) :
  a != b.
Proof.
...
Definition hrellogeq {X : Type} (L R : hrel X) : Type
  := ∏ x1 x2, (L x1 x2 <-> R x1 x2).

Definition istranslogeqf {X : Type} {L R : hrel X}
...
Definition ispologeqf {X : Type} {L R : hrel X} (lg : ∏ x1 x2, L x1 x2 <-> R x1 x2)
...
Definition iseqrellogeqf {X : Type} {L R : hrel X}
...
Definition iscoasymmlogeqf {X : Type} {L R : hrel X}
...
Definition istotallogeqf {X : Type} {L R : hrel X}
...
Definition iscotranslogeqf {X : Type} {L R : hrel X}
...
Definition isdecrellogeqf {X : Type} {L R : hrel X}
...
Definition isnegrellogeqf {X : Type} {L R : hrel X}
...
Definition isantisymmlogeqf {X : Type} {L R : hrel X}
...
Definition isantisymmneglogeqf {X : Type} {L R : hrel X}
...
Definition iscoantisymmlogeqf {X : Type} {L R : hrel X}
...
Definition neqchoicelogeqf {X : Type} {L R : hrel X}
...
(* preoderings *)
Definition po (X : Type) : Type := ∑ R : hrel X, ispreorder R.
Definition make_po {X : Type} (R : hrel X) (is : ispreorder R) : po X
  := tpair ispreorder R is.
Definition carrierofpo (X : Type) : po X -> (X -> X -> hProp) := @pr1 _ ispreorder.
Coercion carrierofpo : po >-> Funclass.

Definition PreorderedSet : Type := ∑ X : hSet, po X.
...
Definition Poset : Type := ∑ X, PartialOrder X.
...
Definition posetmorphism (X Y : Poset) : Type
...
Definition PosetEquivalence (X Y : Poset) : Type
...
Definition isMinimal {X : Poset} (x : X) : Type := ∏ y, x ≤ y.
Definition isMaximal {X : Poset} (x : X) : Type := ∏ y, y ≤ x.
...
Definition make_eqrel {X : Type} (R : hrel X) (is : iseqrel R) : eqrel X
  := tpair (λ R : hrel X, iseqrel R) R is.
Definition eqrelconstr {X : Type} (R : hrel X)
...
Definition hreldirprod {X Y : Type} (RX : hrel X) (RY : hrel Y) :
  hrel (X × Y)
  := λ xy xy' : dirprod X Y, hconj (RX (pr1 xy) (pr1 xy'))
                                    (RY (pr2 xy) (pr2 xy')).

Definition istransdirprod {X Y : Type} (RX : hrel X) (RY : hrel Y)
...
Lemma istransnegrel {X : Type} (R : hrel X) (isr : iscotrans R) :
  istrans (negrel R).
...
Lemma isasymmnegrel {X : Type} (R : hrel X) (isr : iscoasymm R) :
  isasymm (negrel R).
Proof.
  intros. intros x1 x2 r12 r21. apply (r21 (isr _ _ r12)).
Defined.

Lemma iscoasymmgenrel {X : Type} (R : hrel X) (isr : isasymm R) :
  iscoasymm (negrel R).
Proof.
  intros. intros x1 x2 nr12. apply (negf (isr _ _) nr12).
Defined.

Lemma isdecnegrel {X : Type} (R : hrel X) (isr : isdecrel R) :
  isdecrel (negrel R).
...
Lemma isantisymmnegrel {X : Type} (R : hrel X) (isr : isantisymmneg R) :
  isantisymm (negrel R).
Proof.
  intros. apply isr.
Defined.
...
Definition eqh {X : Type} (is : isdeceq X) : hrel X
  := λ x x', make_hProp (booleq is x x' = true)
                        (isasetbool (booleq is x x') true).

Definition neqh {X : Type} (is : isdeceq X) : hrel X
...
Definition weqeqh {X : Type} (is : isdeceq X) (x x' : X) :
  (x = x') ≃ (eqh is x x').
Proof.
...
Definition weqneqh {X : Type} (is : isdeceq X) (x x' : X) :
  (x != x') ≃ (neqh is x x').
Proof.
...
Definition make_decrel {X : Type} {R : hrel X} (is : isdecrel R) : decrel X
  := tpair _ R is.
...
Definition breltodecrel {X : Type} (B : brel X) : decrel X
  := @make_decrel _ (λ x x', make_hProp ((B x x') = true) (isasetbool _ _))
                 (λ x x', (isdeceqbool _ _)).

Definition pathstor {X : Type} (R : decrel X) (x x' : X)
...
Definition rtopaths {X : Type} (R : decrel X) (x x' : X) (r : R x x') :
...
Definition pathstonegr {X : Type} (R : decrel X) (x x' : X)
...
Definition negrtopaths {X : Type} (R : decrel X) (x x' : X) (nr : neg (R x x')) :
...
(** The following construction of "ct" ("canonical term") is inspired by the
  ...
  Definition pathstor_comp {X : Type} (R : decrel X) (x x' : X)
  (e : (decreltobrel R x x') = true) : R x x'.
...
Definition ctlong {X : Type} (R : hrel X) (is : isdecrel R) (x x' : X)
...
Definition deceq_to_decrel {X:Type} : isdeceq X -> decrel X.
Proof.
...
Definition confirm_equal {X : Type} (i : isdeceq X) (x x' : X)
...
Definition confirm_not_equal {X : Type} (i : isdeceq X) (x x' : X)
...
Definition resrel {X : Type} (L : hrel X) (P : hsubtype X) : hrel P
  := λ p1 p2, L (pr1 p1) (pr1 p2).

Definition istransresrel {X : Type} (L : hrel X) (P : hsubtype X)
...
Definition isreflresrel {X : Type} (L : hrel X) (P : hsubtype X)
           (isl : isrefl L) : isrefl (resrel L P).
Proof.
  intros. intro x. apply isl.
Defined.

Definition issymmresrel {X : Type} (L : hrel X) (P : hsubtype X)
           (isl : issymm L) : issymm (resrel L P).
Proof.
  intros. intros x1 x2 r12. apply isl. apply r12.
Defined.

Definition isporesrel {X : Type} (L : hrel X) (P : hsubtype X)
...
Definition iseqrelresrel {X : Type} (L : hrel X) (P : hsubtype X)
...
Definition isantisymmnegresrel {X : Type} (L : hrel X) (P : hsubtype X)
...
Definition iscoantisymmresrel {X : Type} (L : hrel X) (P : hsubtype X)
...
Definition  neqchoiceresrel {X : Type} (L : hrel X) (P : hsubtype X)
...
Definition iseqclassconstr {X : Type} (R : hrel X) {A : hsubtype X}
...
Definition eqax0 {X : Type} {R : hrel X} {A : hsubtype X} :
...
Lemma iseqclassdirprod {X Y : Type} {R : hrel X} {Q : hrel Y}
      {A : hsubtype X} {B : hsubtype Y}
...
*)
Lemma epiissurjectiontosets {A B : Type} (p : A -> B) (isB:isaset B)
...
Definition make_setquot {X : Type} (R : hrel X) (A : hsubtype X)
...
Definition iscomprelfun {X Y : Type} (R : hrel X) (f : X -> Y) : Type
  := ∏ x x' : X, R x x' -> f x = f x'.

Lemma iscomprelfunlogeqf {X Y : Type} {R L : hrel X} (lg : hrellogeq L R)
...
Theorem setquotunivcomm {X : Type} (R : eqrel X) (Y : hSet) (f : X -> Y)
        (is : iscomprelfun R f) :
...
Theorem weqpathsinsetquot {X : Type} (R : eqrel X) (x x' : X) :
...
Definition iscomprelrelfun {X Y : Type} (RX : hrel X) (RY : hrel Y) (f : X -> Y)
  : Type := ∏ x x' : X, RX x x' -> RY (f x) (f x').

Lemma iscomprelfunlogeqf1 {X Y : Type} {LX RX : hrel X} (RY : hrel Y)
...
Lemma iscomprelfunlogeqf2 {X Y : Type} (RX : hrel X) {LY RY : hrel Y}
...
Definition setquotfun {X Y : Type} (RX : hrel X) (RY : eqrel Y) (f : X -> Y)
...
Definition setquotfuncomm {X Y : Type} (RX : eqrel X) (RY : eqrel Y)
...
Theorem setquotuniv2prop {X : Type} (R : eqrel X)
...
Theorem setquotuniv3prop {X : Type} (R : eqrel X)
...
Theorem setquotuniv4prop {X : Type} (R : eqrel X)
...
Lemma issurjsetquotfun {X Y : Type} (RX : eqrel X) (RY : eqrel Y) (f : X -> Y)
...
Lemma isinclsetquotfun {X Y : Type} (RX : eqrel X) (RY : eqrel Y) (f : X -> Y)
...
Definition setquotincl {X Y : Type} (RX : eqrel X) (RY : eqrel Y) (f : X -> Y)
           (is1 : iscomprelrelfun RX RY f)
           (is2 : ∏ x x' : X, RY (f x) (f x') -> RX x x') :
  incl (setquot RX) (setquot RY)
  := make_incl (setquotfun RX RY f is1) (isinclsetquotfun RX RY f is1 is2).

Definition  weqsetquotweq {X Y : Type} (RX : eqrel X) (RY : eqrel Y) (f : X ≃ Y)
            (is1 : iscomprelrelfun RX RY f)
            (is2 : ∏ x x' : X, RY (f x) (f x') -> RX x x') :
...
Definition weqsetquotsurj {X Y : Type} (RX : eqrel X) (RY : eqrel Y) (f : X -> Y)
           (is : issurjective f) (is1 : iscomprelrelfun RX RY f)
           (is2 : ∏ x x' : X, RY (f x) (f x') -> RX x x') :
  (setquot RX) ≃ (setquot RY).
Proof.
...
Definition setquottodirprod {X Y : Type} (RX : eqrel X) (RY : eqrel Y)
...
Definition dirprodtosetquot {X Y : Type} (RX : hrel X) (RY : hrel Y)
           (cd : (setquot RX) × (setquot RY)) :
  setquot (hreldirprod RX RY)
  := make_setquot _ _ (iseqclassdirprod (pr2 (pr1 cd)) (pr2 (pr2 cd))).


Theorem weqsetquottodirprod {X Y : Type} (RX : eqrel X) (RY : eqrel Y) :
...
Definition iscomprelfun2 {X Y : Type} (R : hrel X) (f : X -> X -> Y) : Type
  := ∏ x x' x0 x0' : X, R x x' -> R x0 x0' -> f x x0 = f x' x0'.

Lemma iscomprelfun2if {X Y : Type} (R : hrel X) (f : X -> X -> Y)
...
Lemma iscomprelfun2logeqf {X Y : Type} {L R : hrel X} (lg : hrellogeq L R)
...
Local Lemma setquotuniv2_iscomprelfun {X : Type} (R : hrel X) (Y : hSet) (f : X -> X -> Y)
...
Theorem setquotuniv2comm {X : Type} (R : eqrel X) (Y : hSet) (f : X -> X -> Y)
...
Definition iscomprelrelfun2 {X Y : Type} (RX : hrel X) (RY : hrel Y)
           (f : X -> X -> Y) : Type
  := ∏ x x' x0 x0' : X, RX x x' -> RX x0 x0' -> RY (f x x0) (f x' x0').

Lemma iscomprelrelfun2if {X Y : Type} (RX : hrel X) (RY : eqrel Y)
      (f : X -> X -> Y)
...
Lemma iscomprelrelfun2logeqf1 {X Y : Type} {LX RX : hrel X} (RY : hrel Y)
...
Lemma iscomprelrelfun2logeqf2 {X Y : Type} (RX : hrel X) {LY RY : hrel Y}
...
Local Lemma setquotfun2_iscomprelfun2 {X Y : Type} (RX : hrel X) (RY : eqrel Y)
...
Definition setquotfun2 {X Y : Type} (RX : hrel X) (RY : eqrel Y)
           (f : X -> X -> Y) (is : iscomprelrelfun2 RX RY f)
...
Theorem setquotfun2comm {X Y : Type} (RX : eqrel X) (RY : eqrel Y)
        (f : X -> X -> Y) (is : iscomprelrelfun2 RX RY f) :
...
Theorem isdeceqsetquot_non_constr {X : Type} (R : eqrel X)
...
Definition setquotbooleqint {X : Type} (R : eqrel X)
           (is : ∏ x x' : X, isdecprop (R x x')) (x x' : X) : bool.
Proof.
  intros. induction (pr1 (is x x')). apply true. apply false.
Defined.

Lemma setquotbooleqintcomp {X : Type} (R : eqrel X)
...
Lemma setquotbooleqtopaths {X : Type} (R : eqrel X)
...
Lemma setquotpathstobooleq {X : Type} (R : eqrel X)
      (is : ∏ x x' : X, isdecprop (R x x')) (x x' : setquot R) :
...
Definition isdeceqsetquot {X : Type} (R : eqrel X)
           (is : ∏ x x' : X, isdecprop (R x x')) : isdeceq (setquot R).
Proof.
  intros. intros x x'.
...
Lemma iscomprelrelif {X : Type} {R : hrel X} (L : hrel X) (isr : issymm R)
...
Lemma iscomprelrellogeqf1 {X : Type} {R R' : hrel X} (L : hrel X)
      (lg : hrellogeq R R') (is : iscomprelrel R L) : iscomprelrel R' L.
Proof.
  intros. apply (iscomprelfun2logeqf lg L is).
Defined.

Lemma iscomprelrellogeqf2 {X : Type} (R : hrel X) {L L' : hrel X}
...
Lemma istransquotrel {X : Type} {R : eqrel X} {L : hrel X}
...
Lemma issymmquotrel {X : Type} {R : eqrel X} {L : hrel X} (is : iscomprelrel R L)
...
Lemma isreflquotrel {X : Type} {R : eqrel X} {L : hrel X} (is : iscomprelrel R L)
...
Lemma ispoquotrel {X : Type} {R : eqrel X} {L : hrel X} (is : iscomprelrel R L)
...
Lemma iseqrelquotrel {X : Type} {R : eqrel X} {L : hrel X} (is : iscomprelrel R L)
...
Lemma isirreflquotrel {X : Type} {R : eqrel X} {L : hrel X}
...
Lemma isasymmquotrel {X : Type} {R : eqrel X} {L : hrel X} (is : iscomprelrel R L)
...
Lemma iscoasymmquotrel {X : Type} {R : eqrel X} {L : hrel X}
...
Lemma istotalquotrel {X : Type} {R : eqrel X} {L : hrel X}
...
Lemma iscotransquotrel {X : Type} {R : eqrel X} {L : hrel X}
...
Lemma isantisymmquotrel {X : Type} {R : eqrel X} {L : hrel X}
...
Lemma isantisymmnegquotrel {X : Type} {R : eqrel X} {L : hrel X}
...
Lemma quotrelimpl {X : Type} {R : eqrel X} {L L' : hrel X} (is : iscomprelrel R L)
...
Lemma quotrellogeq {X : Type} {R : eqrel X} {L L' : hrel X}
...
Lemma isdecquotrel {X : Type} {R : eqrel X} {L : hrel X}
...
Definition decquotrel {X : Type} {R : eqrel X} (L : decrel X)
...
Definition reseqrel {X : Type} (R : eqrel X) (P : hsubtype X) : eqrel P :=
  make_eqrel _ (iseqrelresrel R P (pr2 R)).

Lemma iseqclassresrel {X : Type} (R : hrel X) (P Q : hsubtype X)
...
Definition fromsubquot {X : Type} (R : eqrel X) (P : hsubtype (setquot R))
...
Definition tosubquot {X : Type} (R : eqrel X) (P : hsubtype (setquot R)) :
...
Definition weqsubquot {X : Type} (R : eqrel X) (P : hsubtype (setquot R)) :
...
(** Comment: unfortunetely [weqsubquot] is not as useful as it should be at
  ...
  [Lemma test (X : Type) (R : eqrel X) (P : hsubtype (setquot R)) (x : X)
   (is : P (setquotpr R x)) :
  ...
  for [(X : Type) (is : isaprop X)] the term
  ...
  tfc (X : Type) (E : X -> Type) (is : ∏ x, iscontr (E x)) (x0 : X) (e0 : E x0) :
     ∏ x : X, E x.
  ...
  Lemma proofirrelevance {X : Type} (is : isaprop X) : ∏ x1 x2, x1 = x2.
  Proof.
   intros X is x1.
...
Definition pathshrel (X : Type) : X → X → hProp := λ x x' : X, ishinh (x = x').
Definition istranspathshrel (X : Type) : istrans (pathshrel X)
  := λ x x' x'' : _,
       λ a : _,
         λ b : _,
           hinhfun2 (fun e1 : x = x' => fun e2 : x' = x'' => e1 @ e2) a b.
Definition isreflpathshrel (X : Type) : isrefl (pathshrel X)
  := λ x : _, hinhpr (idpath x).
Definition issymmpathshrel (X : Type) : issymm (pathshrel X)
  := λ x x': _, λ a : _, hinhfun (fun e : x = x' => ! e) a.

Definition pathseqrel (X : Type) : eqrel X
  := eqrelconstr (pathshrel X) (istranspathshrel X) (isreflpathshrel X)
                 (issymmpathshrel X).

Definition pi0 (X : Type) : Type := setquot (pathshrel X).
Definition pi0pr (X : Type) : X -> setquot (pathseqrel X)
...
Definition compfun {X : Type} (R : hrel X) (S : Type) : Type
  := total2 (fun F : X -> S => iscomprelfun R F).
Definition make_compfun {X : Type} (R : hrel X) {S : Type} (f : X -> S)
           (is : iscomprelfun R f) : compfun R S := tpair _ f is.
Definition pr1compfun (X : Type) (R : hrel X) (S : Type) :
  @compfun X R S -> (X -> S) := @pr1 _ _.
Coercion pr1compfun : compfun >-> Funclass.

Definition compevmapset {X : Type} (R : hrel X) :
  X -> ∏ S : hSet, (compfun R S) -> S
  := λ x : X, λ S : _, λ f : compfun R S, pr1 f x.

Definition compfuncomp {X : Type} (R : hrel X) {S S' : Type} (f : compfun R S)
           (g : S -> S') : compfun R S'.
Proof.
...
Definition Fi (X Y : Type) (R : hrel X) : X -> F X Y R := λ x, λ f, f x.
...
Definition Fv {X Y : Type} (R : hrel X) (f : compfun R Y) (phi : F X Y R) : Y
 := phi f.

Definition qeq {X Y : Type} (R : hrel X)
...
Lemma isinclpr1qeq {X : Type} (R : hrel X) (Y : hSet) :
...
Lemma iscomptoqeq {X : Type} (Y : hSet) (R : hrel X) :
...
Definition qequniv {X : Type} (Y : hSet) (R : hrel X) (f : compfun R Y)
       (phi : @qeq X Y R) : Y.
Proof.
  intros. apply (Fv R f (pr1 phi)).
Defined.

Lemma qequnivandpr {X : Type} (Y : hSet) (R : hrel X) (f : compfun R Y)
   (x : X) : paths (qequniv Y R f (toqeq R x)) (f x).
Proof.
  intros. apply idpath.
Defined.

Lemma etaqeq {X : Type} (Y : hSet) (R : hrel X) (psi : qeq R -> Y)
...
Definition Fd2 {X Y : Type} (R : hrel X) (phi : F X Y R) (psi : F X Y R -> Y) :
 Y := (Fv R (funcomp (Fi X Y R) psi) phi).

Definition Ffunct {X1 X2 : Type} (f : X1 -> X2) (Y : Type) : F X1 Y -> F X2 Y
 := λ phi, λ g, phi (funcomp f g).



Lemma testd1 {X Y : Type} (psi : F X Y -> Y) (phi : F X Y) :
  paths (psi phi) (Fd1 phi psi).
Proof.
  intros. apply idpath.
Defined.

Lemma testd2 {X Y : Type} (psi : F X Y -> Y) (phi : F X Y) :
...
Definition Fi (X Y : Type) : X -> F X Y := λ x, λ f, f x.
...
Lemma test (X Y : Type) (phi : F X Y) (f : X -> Y) :
...
Inductive try0 (T : Type) (t : T) :
   ∏ (t1 t2 : T) (e1 : t = t1) (e2 : t = t2), Type
   := idconstr : ∏ (t' : T) (e' : t = t'), try0 T t t' t' e' e'.

Definition try0map1 (T : Type) (t : T) (t1 t2 : T) (e1 : t = t1)
   (e2 : t = t2) (X : try0 T t t1 t2 e1 e2) : t1 = t2.
Proof.
  intros. induction  X. apply idpath.
Defined.

Definition try0map2 (T : Type) (t : T) (t1 t2 : T) (e1 : t = t1)
   (e2 : t = t2) : try0 T t t1 t2 e1 e2.
Proof.


Lemma test (X : Type) (t : X) :
...
Lemma test {X : Type} (is : iscontr X) :
...
Lemma test {X : Type} (R : eqrel X) (Y : hSet) (f : setquot R -> Y) :
...
Definition setquot2inset {X : Type} (R : hrel X) : hSet
  := make_hSet _ (isasetsetquot2 R).

(** We will be asuming below that setquot2 is in Type.  In the future it should
  ...
  Y is an hset and X : Type then Y : Type. *)

Definition setquot2pr {X : Type} (R : hrel X) : X -> setquot2 R
...
Definition setquot2univ {X : Type} (R : hrel X) (Y : hSet) (F : X -> Y)
           (is : iscomprelfun R F) (c : setquot2 R) : Y
  := pr1 c Y (make_compfun _ F is).

Theorem setquot2univcomm  {X : Type} (R : hrel X) (Y : hSet) (F : X -> Y)
...
Lemma weqpathssetquot2l1 {X : Type} (R : eqrel X) (x : X) :
...
Theorem weqpathsinsetquot2 {X : Type} (R : eqrel X) (x x' : X) :
...
Definition setquottosetquot2 (X : Type) (R : hrel X) (is : iseqrel R) :
